...
const json_formater = async function Final() {
    ...
    return categoria().then(async (result) => {
        ...
        result['PlazaVea'][10]['Bebidas']['Licores'] = res81.filter((elemento) => elemento != null);

        const res82 = await pa_la_hu('https://www.plazavea.com.pe/bebidas/vinos');
        result['PlazaVea'][10]['Bebidas']['Vinos'] = res82.filter((elemento) => elemento != null);

        const res83 = await pa_la_hu('https://www.plazavea.com.pe/bebidas/espumantes');
        result['PlazaVea'][10]['Bebidas']['Espumantes'] = res83.filter((elemento) => elemento != null);

        const res84 = await pa_la_hu('https://www.plazavea.com.pe/bebidas/cervezas');
        result['PlazaVea'][10]['Bebidas']['Cervezas'] = res84.filter((elemento) => elemento != null);

        const res85 = await pa_la_hu('https://www.plazavea.com.pe/bebidas/gaseosas');
        result['PlazaVea'][10]['Bebidas']['Gaseosas'] = res85.filter((elemento) => elemento != null);

        const res86 = await pa_la_hu('https://www.plazavea.com.pe/bebidas/aguas');
        result['PlazaVea'][10]['Bebidas']['Aguas'] = res86.filter((elemento) => elemento != null);

        const res87 = await pa_la_hu('https://www.plazavea.com.pe/bebidas/jugos-y-otras-bebidas');
        result['PlazaVea'][10]['Bebidas']['Jugos y Otras Bebidas'] = res87.filter((elemento) => elemento != null);

        const res88 = await pa_la_hu('https://www.plazavea.com.pe/bebidas/bebidas-funcionales');
        result['PlazaVea'][10]['Bebidas']['Bebidas Funcionales'] = res88.filter((elemento) => elemento != null);

        //Cuidado del Bebé

        const res91 = await pa_la_hu('https://www.plazavea.com.pe/bebe-e-infantil/panales-y-toallitas-para-bebe');
        result['PlazaVea'][11]['Cuidado del Bebé']['Pañales y Toallitas para Bebé'] = res91.filter((elemento) => elemento != null);

        const res92 = await pa_la_hu('https://www.plazavea.com.pe/bebe-e-infantil/alimentacion-del-bebe');
        result['PlazaVea'][11]['Cuidado del Bebé']['Alimentación del Bebé'] = res92.filter((elemento) => elemento != null);

        const res93 = await pa_la_hu('https://www.plazavea.com.pe/bebe-e-infantil/cuidado-y-aseo-del-bebe');
        result['PlazaVea'][11]['Cuidado del Bebé']['Cuidado y Aseo del Bebé'] = res93.filter((elemento) => elemento != null);

        //Cuidado Personal y Salud

        const res94 = await pa_la_hu('https://www.plazavea.com.pe/cuidado-personal-y-salud/cuidado-del-cabello');
        result['PlazaVea'][12]['Cuidado Personal y Salud']['Cuidado del Cabello'] = res94.filter((elemento) => elemento != null);

        const res95 = await pa_la_hu('https://www.plazavea.com.pe/cuidado-personal-y-salud/higiene-personal');
        result['PlazaVea'][12]['Cuidado Personal y Salud']['Higiene Personal'] = res95.filter((elemento) => elemento != null);

        const res96 = await pa_la_hu('https://www.plazavea.com.pe/cuidado-personal-y-salud/cuidado-bucal');
        result['PlazaVea'][12]['Cuidado Personal y Salud']['Cuidado Bucal'] = res96.filter((elemento) => elemento != null);

        const res97 = await pa_la_hu('https://www.plazavea.com.pe/cuidado-personal-y-salud/cuidado-de-la-piel');
        result['PlazaVea'][12]['Cuidado Personal y Salud']['Cuidado de la Piel'] = res97.filter((elemento) => elemento != null);

        const res98 = await pa_la_hu('https://www.plazavea.com.pe/cuidado-personal-y-salud/proteccion-femenina');
        result['PlazaVea'][12]['Cuidado Personal y Salud']['Protección Femenina'] = res98.filter((elemento) => elemento != null);

        const res99 = await pa_la_hu('https://www.plazavea.com.pe/cuidado-personal-y-salud/proteccion-adulta');
        result['PlazaVea'][12]['Cuidado Personal y Salud']['Protección Adulta'] = res99.filter((elemento) => elemento != null);

        const res100 = await pa_la_hu('https://www.plazavea.com.pe/cuidado-personal-y-salud/afeitado-y-depilacion');
        result['PlazaVea'][12]['Cuidado Personal y Salud']['Afeitado y Depilación'] = res100.filter((elemento) => elemento != null);

        const res103 = await pa_la_hu('https://www.plazavea.com.pe/cuidado-personal-y-salud/salud');
        result['PlazaVea'][12]['Cuidado Personal y Salud']['Salud'] = res103.filter((elemento) => elemento != null);

        const res104 = await pa_la_hu('https://www.plazavea.com.pe/cuidado-personal-y-salud/vitaminas-y-nutricion');
        result['PlazaVea'][12]['Cuidado Personal y Salud']['Vitaminas y Nutrición'] = res104.filter((elemento) => elemento != null);

        //Mercado Saludable

        const res114 = await pa_la_hu('https://www.plazavea.com.pe/mercado-saludable/alimentos-organicos/');
        result['PlazaVea'][15]['Mercado Saludable']['Alimentos Orgánicos'] = res114.filter((elemento) => elemento != null);

        const res117 = await pa_la_hu('https://www.plazavea.com.pe/mercado-saludable/desayunos-organicos/');
        result['PlazaVea'][15]['Mercado Saludable']['Desayunos Orgánicos'] = res117.filter((elemento) => elemento != null);

        const res118 = await pa_la_hu('https://www.plazavea.com.pe/mercado-saludable/snacks-organicos/');
        result['PlazaVea'][15]['Mercado Saludable']['Snacks Orgánicos'] = res118.filter((elemento) => elemento != null);

        const res119 = await pa_la_hu('https://www.plazavea.com.pe/mercado-saludable/vitaminas-y-suplementos-organicos/');
        result['PlazaVea'][15]['Mercado Saludable']['Vitaminas y Suplementos Orgánicos'] = res119.filter((elemento) => elemento != null);
